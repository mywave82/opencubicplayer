...
static void sndhConfigDraw (struct cpifaceSessionAPI_t *cpifaceSession, int focus)
{
	...
	sndhGetStereoModel (&StereoModel, &StereoEmpiricLPF_cutoff, &StereoEmpiricLPF_Qpct);

	if (StereoModel != STEREO_EMPIRIC_LPF)
	{
	...
	if ((sndhConfigHeight >= 4) || (sndhConfigSelected == 0))
	{
		const char *models[] = {"Linear", "Empiric", "Empiric+LPF"};
	...
	if ((sndhConfigHeight >= 3) || (sndhConfigSelected == 1))
	{
		cpifaceSession->console->Driver->DisplayStr (sndhConfigFirstLine + line, 0, (focus && (sndhConfigSelected == 1)) ? 0x07 : 0x08, &"  LPF Frequency:"[2 - skip], 27 + skip);
		DrawBar (cpifaceSession, focus, sndhConfigFirstLine + line, 0 + 27 + skip, sndhConfigWidth - 27 - skip, 1, "Hz", LPF_FREQ_MIN, LPF_FREQ_MAX, StereoEmpiricLPF_cutoff, sndhConfigSelected == 1, StereoModel != STEREO_EMPIRIC_LPF);
		line++;
		if (line >= sndhConfigHeight)
		{
			return;
	...
	if ((sndhConfigHeight >= 2) || (sndhConfigSelected == 2))
	{
		cpifaceSession->console->Driver->DisplayStr (sndhConfigFirstLine + line, 0, (focus&&(sndhConfigSelected == 2)) ? 0x07 : 0x08, &"  LPF Q-factor:"[2 - skip], 27 + skip);	
		DrawBar (cpifaceSession, focus, sndhConfigFirstLine + line, 0 + 27 + skip, sndhConfigWidth - 27 - skip, 100, "", LPF_QPCT_MIN, LPF_QPCT_MAX, StereoEmpiricLPF_Qpct, sndhConfigSelected == 2, StereoModel != STEREO_EMPIRIC_LPF);
		line++;
		if (line >= sndhConfigHeight)
		{
			return;
...
static int sndhConfigAProcessKey (struct cpifaceSessionAPI_t *cpifaceSession, uint16_t key)
{
	...
	if ((key != KEY_LEFT) && (key != KEY_RIGHT) && (key != lastkey))
	{
		...
		if ((newpress-lastpress) > 250) /* 125 ms */
		{
			...
			{
				warmup++;
			} else if ((sndhConfigSelected == 1) && (repeat < 100))
			{
				repeat += 2;
			} else if ((sndhConfigSelected == 2) && (repeat < 5))
			{
				repeat++;
				warmup = 3;
			}
	...
	switch (key)
	{
		...
		case KEY_DOWN:
			if ((StereoModel == STEREO_EMPIRIC_LPF) && (sndhConfigSelected < 2))
			{
				sndhConfigSelected++;
			}
			break;
		...
		case KEY_LEFT:
			...
			sndhSetStereoModel (cpifaceSession, StereoModel, StereoEmpiricLPF_cutoff, StereoEmpiricLPF_Qpct);
			break;
