...
#define instr(opcode,cycles) case opcode: {ay_tstates+=cycles
#define HLinstr(opcode,cycles,morecycles) \
                             case opcode: {uint16_t addr; \
                                ay_tstates+=cycles; \
                                if(ixoriy==0)addr=hl; \
                                else ay_tstates+=morecycles, \
                                   addr=(ixoriy==1?ix:iy)+ \
                                        (int8_t)fetch(pc),\
                                   pc++
...
#define addhl(hi,lo) /* 16-bit add */ if(!ixoriy){\
                   ...
                   else do{uint32_t t=(ixoriy==1?ix:iy);\
                      f=(f&0xc4)|(((t&0xfff)+((hi<<8)|lo)>0xfff)<<4);\
                      t+=(hi<<8)|lo;\
                      if(ixoriy==1)ix=t; else iy=t;\
                      f|=((t>>8)&0x28)|(t>>16);\
                   } while(0)
#define adda(x,c) /* 8-bit add */ do{uint16_t y;\
                      uint8_t z=(x);\
                      y=a+z+(c);\
...
#define suba(x,c) /* 8-bit subtract */ do{uint16_t y;\
                      uint8_t z=(x);\
                      y=(a-z-(c))&0x1ff;\
...
#define cpa(x) /* 8-bit compare */ do{uint16_t y;\
                      uint8_t z=(x);\
                      y=(a-z)&0x1ff;\
...
#define jr /* execute relative jump */ do{int j=(int8_t)fetch(pc);\
                      pc+=j+1;\
                      ay_tstates+=5;\
                   } while(0)
...
instr(0xd3,11);
   ay_tstates+=ay_out(a,fetch(pc),a);
   pc++;
endinstr;
