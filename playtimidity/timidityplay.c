...
/* timidity.cfg or _timidity.cfg or .timidity.cfg*/
    snprintf(path, sizeof (path), "%s" PATH_STRING "timidity.cfg", cpifaceSession->configAPI->HomePath); /* Vanilla TiMidity only scans this on WIN32 */
    status = read_config_file(&tc, path, 0, 1);
    if (status != 3 /* READ_CONFIG_FILE_NOT_FOUND */ )
        return status;

    snprintf(path, sizeof (path), "%s" PATH_STRING "_timidity.cfg", cpifaceSession->configAPI->HomePath); /* Vanilla TiMidity only scans this on WIN32 */
    status = read_config_file(&tc, path, 0, 1);
    if (status != 3 /* READ_CONFIG_FILE_NOT_FOUND */)
        return status;
...
static int emulate_main_start(struct timiditycontext_t *c, struct cpifaceSessionAPI_t *cpifaceSession)
{
	...
	if (!c->got_a_configuration)
	{
		...
		{
			c->got_a_configuration = 1;
		} else if (!read_config_file (&tc, "C:\\timidity\\timidity.cfg", 0, 0))
		{
			c->got_a_configuration = 1;
		} else if (!read_config_file (&tc, "C:\\TiMidity++\\timidity.cfg", 0, 0))
		{
			c->got_a_configuration = 1;
		} else {
