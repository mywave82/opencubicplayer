...
static void timidityConfigDraw (int EditPos, const struct DevInterfaceAPI_t *API)
{
	...
	if (!configfile[0])
	{
		API->console->DisplayPrintf (mlTop++, mlLeft, 0x03, mlWidth, "    (Global default)%*C %.9o\xb3", mlWidth - 21 - 1);
		API->console->DisplayPrintf (mlTop++, mlLeft, (EditPos==0)?0x87:0x07, mlWidth, "    Select another file%0.7o%*C %.9o\xb3", (EditPos==0)?8:0, mlWidth - 23 - 2);
	} else {
