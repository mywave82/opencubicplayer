...
########## LIBJPEG-TURBO ##########
wget https://sourceforge.net/projects/libjpeg-turbo/files/2.1.5.1/libjpeg-turbo-2.1.5.1.tar.gz/download -O libjpeg-turbo-2.1.5.1.tar.gz
rm -Rf libjpeg-turbo-2.1.5.1
tar xfz libjpeg-turbo-2.1.5.1.tar.gz
cd libjpeg-turbo-2.1.5.1
do_cmake -DENABLE_SHARED=TRUE
cd ..

########## LIBPNG ##########
wget https://download.sourceforge.net/libpng/libpng-1.6.39.tar.gz -O libpng-1.6.39.tar.gz
rm -Rf libpng-1.6.39
tar xfz libpng-1.6.39.tar.gz
cd libpng-1.6.39
...
########## FLAC ##########
wget https://ftp.osuosl.org/pub/xiph/releases/flac/flac-1.4.2.tar.xz -O flac-1.4.2.tar.xz
rm -Rf flac-1.4.2
tar xfJ flac-1.4.2.tar.xz
cd flac-1.4.2
do_cmake -D_OGG_LIBRARY_DIRS=$prefix/lib
cd ..

########## SDL2 ##########
wget https://github.com/libsdl-org/SDL/releases/download/release-2.26.5/SDL2-devel-2.26.5-mingw.tar.gz -O SDL2-devel-2.26.5-mingw.tar.gz
rm -Rf SDL2-2.26.5
tar xfz SDL2-devel-2.26.5-mingw.tar.gz
cd SDL2-2.26.5
cp $host/* $prefix -R
...
########## HARFBUZZ #########
wget https://github.com/harfbuzz/harfbuzz/releases/download/7.3.0/harfbuzz-7.3.0.tar.xz -O harfbuzz-7.3.0.tar.xz
rm -Rf harfbuzz-7.3.0
tar xfJ harfbuzz-7.3.0.tar.xz
cd harfbuzz-7.3.0
do_cmake -DCMAKE_CXX_FLAGS=-Wa,-mbig-obj
cd ..

########## FREETYPE2 ##########
wget https://sourceforge.net/projects/freetype/files/freetype2/2.13.0/freetype-2.13.0.tar.gz/download -O freetype-2.13.0.tar.gz
rm -Rf freetype-2.13.0
tar xfz freetype-2.13.0.tar.gz
cd freetype-2.13.0
do_cmake -DZLIB_ROOT=/usr/$host/ \
...
########## cJSON ##########
wget https://github.com/DaveGamble/cJSON/archive/refs/tags/v1.7.15.tar.gz -O cJSON-1.7.15.tar.gz
rm -Rf cJSON-1.7.15
tar xfz cJSON-1.7.15.tar.gz
cd cJSON-1.7.15
patch -p 1 <<EOF
...
########## ancient ##########
wget https://github.com/temisu/ancient/archive/refs/tags/v2.0.0.tar.gz -O ancient-2.0.0.tar.gz
rm -Rf ancient-2.0.0
tar xfz ancient-2.0.0.tar.gz
cd ancient-2.0.0
mkdir -p m4
...
cp $host-src/unifont-15.0.06.otf       $install/unifont.otf
cp $host-src/unifont_csur-15.0.06.otf  $install/unifont_csur.otf
cp $host-src/unifont_upper-15.0.06.otf $install/unifont_upper.otf
