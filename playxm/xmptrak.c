...
static int xm_getnote (struct cpifaceSessionAPI_t *cpifaceSession, uint16_t *bp, int small)
{
	...
	switch (small)
	{
		case 0:
			if (note==96)
				cpifaceSession->conFunc->WriteString (bp, 0, COLINS, "---", 3);
			else {
				...
				cpifaceSession->conFunc->WriteString (bp, 2, porta?COLPTNOTE:COLNOTE, &"01234567"[note/12], 1);
			}
			break;
		case 1:
			if (note==96)
				cpifaceSession->conFunc->WriteString (bp, 0, COLINS, "--", 2);
			else {
				cpifaceSession->conFunc->WriteString (bp, 0, porta?COLPTNOTE:COLNOTE, &"cCdDefFgGaAb"[note%12], 1);
				cpifaceSession->conFunc->WriteString (bp, 1, porta?COLPTNOTE:COLNOTE, &"01234567"[note/12], 1);
			}
			break;
		case 2:
			if (note==96)
				cpifaceSession->conFunc->WriteString (bp, 0, COLINS, "-", 1);
			else
				cpifaceSession->conFunc->WriteString (bp, 0, porta?COLPTNOTE:COLNOTE, &"cCdDefFgGaAb"[note%12], 1);
			break;
	}
...
static int xm_getvol (struct cpifaceSessionAPI_t *cpifaceSession, uint16_t *bp)
{
	...
	{
		cpifaceSession->conFunc->WriteNum (bp, 0, COLVOL, vol-0x10, 16, 2, 0);
		return 1;
	}
	if (xmcurpat[xmcurchan][3]==xmpCmdVolume)
	{
		cpifaceSession->conFunc->WriteNum (bp, 0, COLVOL, xmcurpat[xmcurchan][4], 16, 2, 0);
		return 1;
	}
	return 0;
}
...
static int xm_getpan (struct cpifaceSessionAPI_t *cpifaceSession, uint16_t *bp)
{
	...
	{
		cpifaceSession->conFunc->WriteNum (bp, 0, COLPAN, (xmcurpat[xmcurchan][2]&0xF)*0x11, 16, 2, 0);
		return 1;
	}
	if (xmcurpat[xmcurchan][3]==xmpCmdPanning)
	{
		cpifaceSession->conFunc->WriteNum (bp, 0, COLPAN, xmcurpat[xmcurchan][4], 16, 2, 0);
		return 1;
	}
	if (xmcurpat[xmcurchan][3]==xmpCmdSPanning)
	{
		cpifaceSession->conFunc->WriteNum (bp, 0, COLPAN, xmcurpat[xmcurchan][4]*0x11, 16, 2, 0);
		return 1;
	}
	return 0;
}
...
static void xm_getfx (struct cpifaceSessionAPI_t *cpifaceSession, uint16_t *bp, int n)
{
	...
	switch (xmcurpat[xmcurchan][2]>>4)
	{
		...
		case xmpVCmdFVolSlideU:
			...
			break;
		case xmpVCmdVibRate:
			cpifaceSession->conFunc->WriteString (bp, 0, COLPITCH, "~\x1A", 2);
			cpifaceSession->conFunc->WriteNum    (bp, 2, COLPITCH, data, 16, 1, 0);
			break;
		case xmpVCmdVibDep:
	...
	switch (xmcurpat[xmcurchan][3])
	{
		...
		case xmpCmdKeyOff:
			cpifaceSession->conFunc->WriteString (bp, 0, COLINS, "-", 1);
			cpifaceSession->conFunc->WriteNum    (bp, 1, COLINS, data&0xF, 16, 2, 0);
			break;
		case xmpCmdEnvPos:
			cpifaceSession->conFunc->WriteString (bp, 0, COLINS, "\x1A", 1);
			cpifaceSession->conFunc->WriteNum    (bp, 1, COLINS, data, 16, 2, 0);
			break;
		...
		case xmpCmdFPortaU:
			cpifaceSession->conFunc->WriteString (bp, 0, COLPITCH, "+", 1);
			cpifaceSession->conFunc->WriteNum    (bp, 1, COLPITCH, data*4, 16, 2, 0);
			break;
		case xmpCmdFPortaD:
			cpifaceSession->conFunc->WriteString (bp, 0, COLPITCH, "-", 1);
			cpifaceSession->conFunc->WriteNum    (bp, 1, COLPITCH, data*4, 16, 2, 0);
		...
		case xmpCmdPanSlide:
			if (!data)
				cpifaceSession->conFunc->WriteString (bp, 0, COLPAN, "\1D""00", 3);
			else
				if (data&0xF0)
				{
					cpifaceSession->conFunc->WriteString (bp, 0, COLPAN, "\x1A", 1);
		...
		case xmpCmdXPorta:
			...
			{
				cpifaceSession->conFunc->WriteString (bp, 0, COLPITCH, "+", 1);
				cpifaceSession->conFunc->WriteNum    (bp, 1, COLPITCH, data&0xF, 16, 2, 0);
			} else
				if ((data>>4)==2)
				{
					cpifaceSession->conFunc->WriteString (bp, 0, COLPITCH, "-", 1);
		...
		case xmpCmdGlissando:
			if (data)
				cpifaceSession->conFunc->WriteString (bp, 0, COLPITCH, "\x0D=\xA9", 3);
			else
				cpifaceSession->conFunc->WriteString (bp, 0, COLPITCH, "\x0D=/", 3);
			break;
		case xmpCmdVibType:
			cpifaceSession->conFunc->WriteString (bp, 0, COLPITCH, "~=", 2);
			cpifaceSession->conFunc->WriteString (bp, 2, COLPITCH, &"~\\\xA9?"[data&3], 1);
			break;
		case xmpCmdTremType:
			cpifaceSession->conFunc->WriteString (bp, 0, COLVOL, "~=", 2);
			cpifaceSession->conFunc->WriteString (bp, 2, COLVOL, &"~\\\xA9?"[data&3], 1);
			break;
		case xmpCmdSFinetune:
			cpifaceSession->conFunc->WriteString (bp, 0, COLINS, "ft", 2);
			cpifaceSession->conFunc->WriteNum    (bp, 2, COLINS, data, 16, 1, 0);
			break;
	}
}
...
static void xm_getgcmd (struct cpifaceSessionAPI_t *cpifaceSession, uint16_t *bp, int n)
{
	...
	for (i=0; i<mod.nchan; i++)
	{
		...
		switch (xmcurpat[i][3])
		{
			...
			case xmpCmdSpeed:
				if (!data)
					cpifaceSession->conFunc->WriteString (bp, 0, COLACT, "end", 3);
				else
					if (data<0x20)
					{
						cpifaceSession->conFunc->WriteString (bp, 0, COLSPEED, "t", 1);
			...
			case xmpCmdPatLoop:
				...
				break;
			case xmpCmdPatDelay:
				cpifaceSession->conFunc->WriteString (bp, 0, COLACT, "pd", 2);
				cpifaceSession->conFunc->WriteNum    (bp, 2, COLACT, data, 16, 1, 0);
				break;
			default:
				bp-=4;
